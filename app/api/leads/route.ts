...
// Helper to parse birth coordinates string "e.g. 15 Oct 1990, 08:30 AM, New Delhi"
function parseBirthCoordinates(raw: string) {
  if (!raw) return null;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('application/x-www-form-urlencoded')) {
      ...
      birthCoordinatesRaw = formData.get('birth_coordinates') as string || '';
      ...
      phone = body.phone || '';
      city = body.city || '';
      birthCoordinatesRaw = body.birth_coordinates || '';
    ...
    const birthDetails = parseBirthCoordinates(birthCoordinatesRaw);
    ...
    if (insertError) {
      ...
      return NextResponse.json({ error: 'Failed to save lead details' }, { status: 500 });
    }
    ...
    if (lead) {
      const { error: logError } = await supabase
        ...
        .insert({
          ...
          description: `New lead from website contact form: ${name}`
        });

      if (logError) {
        console.error('[API Leads] Supabase activity log insert error:', logError);
      }
    }
    ...
    if (email) {
      sendThankYouEmail(name, email).catch(err => {
        console.error('[API Leads] Async email delivery error:', err);
      });
    }

    if (isFormSubmission) {
      // Redirect back to homepage with success query param
